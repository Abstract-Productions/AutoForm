<?php
  ...
  class DynamicContent {
    ...
    public function __invoke($obj):string {
      if (is_callable($this->value)) { 
        ob_start();
        return (($this->value)($obj) ?? "").ob_get_clean();
      }
  ...
  class AutoForm {
    public DynamicContent $header;
    public DynamicContent $footer;
    public array $Input;
    ...
    public bool $ajax;
    public bool $ajax_as_you_go;
    public string $submit_label;
    public array $errors = [];
    public array $fields = [];
    public int $robot_delay;
    public int $pages;
    
    public array $Province_List;
    public array $State_List;
    public array $Country_List;

    public bool $form_valid = true;
    ...
    public function __construct(array $Options = []) {
      ...
      $this->set_head($Options['header'] ?? "");
      $this->set_foot($Options['footer'] ?? "");
      $this->robot_delay = $Options['robot_delay'] ?? 5;
      $this->pages = 1;
      if (empty($this->Input['af_page'])) $this->Input['af_page'] = 1;
      $this->Province_List = $this->make_list($this->province_string);
      $this->State_List = $this->make_list($this->state_string);
      $this->Country_List = $this->make_list($this->country_string);
    }
      ...
      if (in_array($this->Input['af_action'] ?? "", [hash("sha256", "continue$_SESSION[start_timestamp]"), hash("sha256", "ajax$_SESSION[start_timestamp]")])) {
        ...
        if (empty($this->errors)) {
          if ($this->Input['af_page'] == $this->pages) return true;
            ...
            $this->set_inp("af_page", $this->Input['af_page'] + 1);
            $this->set_inp("af_action", "");
          }
        }
      }
      $this->display_form();
    ...
    public function head():bool {
      print ($this->header)($this);
      return true;
    }

    public function foot():bool {
      print ($this->footer)($this);
      return true;
    }

    public function wrap(string|callable $content):bool {
      $this->head();
      print new DynamicContent($content)($this);
      $this->foot();
      return true;
    }

    public function set_head(string|callable $header):DynamicContent {
      return $this->header = new DynamicContent($header);
    }

    public function set_foot(string|callable $footer):DynamicContent {
      return $this->footer = new DynamicContent($footer);
    }
    ...
    public function display_form() {
      ...
      foreach ($this->fields as $Field) {
        if ($Field['type'] == "html") {
          if ($Field['page'] == $this->Input['af_page']) print $Field['html']($this);
        }
        else {
          $labeled_by = "";
    ...
    public function add_fields(array $Arr = []):bool {
      foreach ($Arr as $field_name => $Info) {
        if (is_string($Info) || is_callable($Info)) $field_ok = $this->add_html($Info);
        else $field_ok = $this->add_field($field_name, $Info['type'] ?? "text", $Info);
        if (!$field_ok) return false;
      }
      return true;
    }

    public function add_field(string $field_name, string $type = "text", string|array $label = NULL, $validate = false, array $values = [], string $default = "", array $Attributes = []):bool {
    ...
    public function new_page():bool {
      if (empty($this->fields) || end($this->fields)['page'] < $this->pages)
        return $this->warn("Called new_page when current page is blank ".end($this->fields)['page']);
      $this->pages++;
      return true;
    }

    public function add_select(string $field_name, string|array $label = "", $validate = false, array $values = [], string $default = "", array $Attributes = []):bool {
      return $this->add_field($field_name, "select", $label, $validate, $values, $default, $Attributes);
    }

    public function add_text(string $field_name, string $label = "", $validate = false, array $values = [], string $default = "", array $Attributes = []):bool {
      return $this->add_field($field_name, "text", $label, $validate, $values, $default, $Attributes);
    }
    
    public function add_textarea(string $field_name, string $label = "", $validate = false, array $values = [], string $default = "", array $Attributes = []):bool {
      return $this->add_field($field_name, "textarea", $label, $validate, $values, $default, $Attributes);
    }

    public function add_radio(string $field_name, string|array $label = "", $validate = false, array $values = [], string $default = "", array $Attributes = []):bool {
      return $this->add_field($field_name, "radio", $label, $validate, $values, $default, $Attributes);
    }

    public function add_checkbox(string $field_name, string|array $label = "", $validate = false, $values = [], $default = "", $Attributes = []):bool {
      return $this->add_field($field_name, "checkbox", $label, $validate, $values, $default, $Attributes);
    }

    public function add_html(string|callable $html):true {
      $this->fields[] = [
        "type" => "html",
        "html" => new DynamicContent($html),
        "page" => $this->pages
      ];
      return true;
    }

    public function add_validation(string $field_name, $validate):bool {
    ...
    public function att($Attribs = []):string {
      $att_string = "";
    ...
    public function tag($tag_name, $Attribs = [], $close_after = NULL, $xtra = ""):string {
      $tag_string = "<$tag_name".$this->att($Attribs)."$xtra>";
      if (!is_null($close_after)) $tag_string .= "$close_after</$tag_name>";
      return "$tag_string\n";
    }

    public function set_inp($var, $val) {
      $this->Input[$var] = $val;
      $this->Sanitized[$var] = $this->sanitize($val);
      return $val;
    }

    public function unset_inp($var):NULL {
      unset($this->Input[$var], $this->Sanitized[$var]);
      return NULL;
    }

    private function warn($message):false {
      trigger_error($message, E_USER_WARNING);
      return $this->form_valid = false;
    }

    private function make_list(string $list):array {
      $Arr = [];
      $Lines = explode("\n", $list);
    ...
    private string $province_string = "AB|Alberta
BC|British Columbia
MB|Manitoba
NB|New Brunswick
...
YT|Yukon";
    ...
    private string $state_string = "AK|Alaska
AL|Alabama
AR|Arkansas
AS|American Samoa
AZ|Arizona
...
WY|Wyoming";
    ...
    private $country_string = "AF|Afghanistan
AL|Albania
DZ|Algeria
AS|American Samoa
AD|Andorra
